...
public class PatientRepository {
    ...
    private static final PatientRepository intance = new PatientRepository();
...
//        생성(Create)와 수정(Update)가 같은 save 메서드를 사용
        Optional<Patient> existing = findById(patient.getId());
//        existing.ifPresent(value -> this.delete(value));
        existing.ifPresent(this::delete); // 전달받은 환자의 ID가 이미 존재하는 경우 삭제 (존재하지 않는 경우에는 아래 코드로 진행)
