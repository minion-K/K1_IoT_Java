...
*
* 1. throw
* : 프로그램 실행 중에 특정 조건에서 예외를 발생시킬 때 사용
* > 주로 조건문과 함께 사용 (프로그램의 흐름을 제어하는 역할)
* 
* [ 사용법 ]
* : throw new ExceptionType("예외 메시지");
* - 인자로 전달하는 문자열은
* - 예외변수(e).getMassage() 메서드를 활용하여 catch 블록에서 출력 가능
*
* 2. throws
* : 메서드 선언부에 위치, 호출자에게 예외 처리를 위임
* - 체크(컴파일) 예외: 필수 명시 / 언체크(런타임): 선택 명시
*
* [ 사용법 ]
...
//        try-catch 블록은 항상 같이 사용
        try {
            ...
            System.out.println("예외 발생 시 실행 X / 예외 발생 하지 않으면 실행 O");
        } catch (Exception e) {
