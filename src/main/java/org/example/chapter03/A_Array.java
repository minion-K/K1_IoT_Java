...
//          : 데이터를 "연속적"으로 저장
//          - 순서 O (index 인덱스 번호 - 0부터 시작하여 1씩 증가하는 값)
//          - 중복 0 (동일한 값을 여러 요소에 저장 가능)

//        cf) 요소: 배열 내에서 각각의 데이터를 의미
...
//        == 배열의 생성 (우항) ==
//         : new 데이터타입[배열의 길이]
        scores1 = new int[3]; // 요소의 개수가 3개인 배열 생성
...
//        : 배열의 크기 값을 초과할 경우 접근 불가! 예외 발생

//        1) 배열의 선언: int[] numbers;
//        2) 배열의 생성: numbers = new int[3];
//        3) 배열의 초기화: 각각의 위치 인덱스 번호를 사용하여 데이터 삽입

//        == 배열의 선언과 동시에 초기화 ==
//        : new 연산자 사용 + 직접으로 요소를 대입하여 배열 생성 시 길이 지정 X
...
//        cf) 참조 자료형의 출력 
//         : 자바에서 '객체'를 출력하면 system.out으로 출력하면 내부적으로 객체.toString() 기능을 호출
//         >> 모든 참조 자료형의 집합체
