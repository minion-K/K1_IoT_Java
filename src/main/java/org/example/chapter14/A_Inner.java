...
//              >> 재할당이 이루어지면 final이 사라짐 (final로 취급되지 않는다)
...
//        로컬 클래스의 인스턴스화
//        : 메서드 내부에서만 쓰이기 때문에 인스턴스화도 내부에서 일어만
        MethodClass methodClass = new MethodClass();
        methodClass.display();
    }
}

// 추상클래스 (하나 이상의 추상 메서드 포함)
...
//        1) 외부 클래스 인스턴스화 필요 X
//              >> 외부클래스타입.내부클래스 변수명 = new 외부클래스명.내부클래스
