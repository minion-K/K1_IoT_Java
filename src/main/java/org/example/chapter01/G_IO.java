...
//        cf) 개행 문자(Enter)
//        : 줄을 바꿀 때 사용되는 특수 문자
//        : 사용자가 Enter 키를 입력하면 개행 문자가 입력됨

//        1) nextInt(), nextDouble()은 숫자만 가져감
//              -next + 데이터타입은 해당 데이터만 인식

//        2) nextLine()은 줄 전체를 가져감
//              [문제 상황]
//              - 키(height) 입력 시 실제 Scanner 내부에는 키 값 + 개행문자가 저장
//              - nextInt()에서 키 값만 가져가고 개행문자는 그대로 버퍼에 날림
//              - nextLine()은 사용자의 입력 값을 기다리지 않고 빈 문자열(개행문자)를 담고 종료
