...
//  === 제어문 ===
// : 프로그램 실행 흐름을 제어(Control)하는 문법

// 1) 조건문(Condition)
// : 특정 조건을 만족할 때만 코드 블록을 실행
// - if, else, else if: 조건에 따라 실행할 코드를 결정
// - switch, case, default: 여러 개의 경우(case)에 따라 실행할 코드를 결정
...
//            조건식이 거짓인 경우 (age는 19 이하의 모든 수)
            System.out.println("미성년자입니다.");
        }

//        예제2) 나이 분류 (else if 추가)
        if (age > 19 ) {
...
//        3. 각 분기에 위치할 경우 functionsScope 변수에 몇 사분면인지 (재)할당
...
//        == 풀이 ==
//        1. 각 조건식에서 저장할 문자열 값 (사분면의 위치)을 담을 변수 선언
...
//        2. 스캐너 생성
        ...
        int y = sc.nextInt();
//        3. 조건문 작성
        if(x == 0 && y == 0) functionScope = "원점";
        else if (x > 0 && y > 0 ) functionScope = "제 1사분면";
        else if (x < 0 && y > 0) functionScope = "제 2사분면";
        else if (x < 0 && y < 0) functionScope = "제 3사분면";
        else if (x > 0 && y < 0 ) functionScope= "제 4사분면";
        else if (x == 0 && y != 0) functionScope = "y축 위의 좌표";
        else  functionScope = "x축 위의 좌표";
//        4. functionScope 값 출력
        System.out.println("결과: " + functionScope + "입니다.");
//        5. 스캐너 종료
        sc.close();
    }
}
