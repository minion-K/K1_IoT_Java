...
//    정적 메서드: 재정의(오버라이딩) 불가능
    static void printType() {
        System.out.println("과일(Fruit) 타입 입니다.");
    }
}
...
// cf) implements: A(클래스) implements B(인터페이스)
...
//        > TropicalFrut가 Fruit를 상속받는 인터페이스
...
//        'A' 객체가 'B' 클래스(종류)와 같은지 확인
        ...
        if(나비 instanceof 고양이) {
            ...
            고양이 야옹 = (고양이) 나비; // if 조건문 외부에서도 가능! 다만 확실하게 검증하고 반환
        }
    }
}
